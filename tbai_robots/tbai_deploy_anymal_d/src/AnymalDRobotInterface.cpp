...
void AnymalDRobotInterface::initMotorMapping() {
...
AnymalDRobotInterface::AnymalDRobotInterface(AnymalDRobotInterfaceArgs args) {
    logger_ = tbai::getLogger("tbai_deploy_anymal_d");
    TBAI_LOG_INFO(logger_, "AnymalDRobotInterface constructor (tbai_sdk/zenoh backend)");

    initMotorMapping();

    TBAI_LOG_INFO(logger_, "Initializing publisher: Topic: {}", ANYMAL_D_TOPIC_LOWCMD);
    lowcmd_publisher = std::make_unique<tbai::Publisher<robot_msgs::MotorCommands>>(ANYMAL_D_TOPIC_LOWCMD);
...
AnymalDRobotInterface::~AnymalDRobotInterface() {
    TBAI_LOG_INFO(logger_, "Destroying AnymalDRobotInterface");
    if (lowstate_subscriber) lowstate_subscriber->stop();
}

void AnymalDRobotInterface::lowStateCallback(const robot_msgs::LowState &low_state) {
    ...
    vector_t jointAngles(ANYMAL_D_NUM_JOINTS);
    vector_t jointVelocities(ANYMAL_D_NUM_JOINTS);
    ...
    State state;
    state.x = vector_t::Zero(ANYMAL_D_STATE_DIM);
...
void AnymalDRobotInterface::publish(std::vector<MotorCommand> commands) {
    ...
    motor_commands.commands.resize(ANYMAL_D_NUM_JOINTS);
...
void AnymalDRobotInterface::waitTillInitialized() {
    while (!initialized_) {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        TBAI_LOG_INFO_THROTTLE(logger_, 1.0, "Waiting for the ANYmal D robot to initialize...");
    }
    TBAI_LOG_INFO(logger_, "ANYmal D robot initialized");
}

State AnymalDRobotInterface::getLatestState() {
    std::lock_guard<std::mutex> lock(latestStateMutex_);
    return state_;
}

void AnymalDRobotInterface::updateGroundPlaneEstimate() {
...
double AnymalDRobotInterface::estimateGroundHeight(const robot_msgs::PointCloud2 &pc) {
