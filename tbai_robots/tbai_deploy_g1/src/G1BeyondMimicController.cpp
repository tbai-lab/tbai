...
G1BeyondMimicController::G1BeyondMimicController(const std::shared_ptr<tbai::RobotInterface> &robotInterfacePtr,
    ...
    observation_ = vector_t::Zero(BEYOND_TOTAL_OBS_SIZE);
    ...
    // Load configuration
    defaultJointPos_ = tbai::fromGlobalConfig<vector_t>("g1_beyond_mimic_controller/default_joint_pos");
    actionScale_ = tbai::fromGlobalConfig<vector_t>("g1_beyond_mimic_controller/action_scale");
    stiffness_ = tbai::fromGlobalConfig<vector_t>("g1_beyond_mimic_controller/stiffness");
    damping_ = tbai::fromGlobalConfig<vector_t>("g1_beyond_mimic_controller/damping");
    ...
    jointIdsMap_ = tbai::fromGlobalConfig<std::vector<int>>("g1_beyond_mimic_controller/joint_ids_map");
    jointNames_ = tbai::fromGlobalConfig<std::vector<std::string>>("joint_names");
    maxTimestep_ = tbai::fromGlobalConfig<int>("g1_beyond_mimic_controller/max_timestep", -1);
    actionBeta_ = tbai::fromGlobalConfig<float>("g1_beyond_mimic_controller/action_beta", actionBeta);
...
G1BeyondMimicController::~G1BeyondMimicController() {
...
void G1BeyondMimicController::initOnnxRuntime(const std::string &policyPath) {
    TBAI_LOG_INFO(logger_, "Loading ONNX policy from: {}", policyPath);

    ortEnv_ = std::make_unique<Ort::Env>(ORT_LOGGING_LEVEL_WARNING, "G1BeyondMimicController");
...
void G1BeyondMimicController::parseModelMetadata() {
...
quaternion_t G1BeyondMimicController::computeAnchorOrientation(const quaternion_t &rootQuat,
...
Eigen::Matrix<scalar_t, 6, 1> G1BeyondMimicController::computeOrientationError(const quaternion_t &targetQuat,
...
void G1BeyondMimicController::buildObservation(scalar_t currentTime, scalar_t dt) {
    ...
    // Need to map from DDS order to BeyondMimic policy order
...
void G1BeyondMimicController::runInference() {
    std::vector<int64_t> obsShape = {1, BEYOND_TOTAL_OBS_SIZE};
    std::vector<float> obsData(BEYOND_TOTAL_OBS_SIZE);
    for (int i = 0; i < BEYOND_TOTAL_OBS_SIZE; ++i) {
    ...
    if (outputTensors.size() > 1) {
        ...
        if (outputTensors.size() > 3) {
            ...
            for (int i = 0; i < BEYOND_NUM_BODIES; ++i) {
...
std::vector<tbai::MotorCommand> G1BeyondMimicController::getMotorCommands(scalar_t currentTime, scalar_t dt) {
...
bool G1BeyondMimicController::isSupported(const std::string &controllerType) {
    return controllerType == controllerName_ || controllerType == "G1BeyondMimicController" ||
           controllerType == "g1_beyond_mimic" || controllerType == "beyond_mimic";
}

void G1BeyondMimicController::stopController() {
    motionActive_ = false;
    TBAI_LOG_INFO(logger_, "{} stopped", controllerName_);
}

bool G1BeyondMimicController::ok() const {
    return true;
}

bool G1BeyondMimicController::checkStability() const {
...
bool G1BeyondMimicController::isMotionComplete() const {
    return maxTimestep_ > 0 && timestep_ >= maxTimestep_;
}

void G1BeyondMimicController::changeController(const std::string &controllerType, scalar_t currentTime) {
