...
void AnymalBRobotInterface::initMotorMapping() {
...
AnymalBRobotInterface::AnymalBRobotInterface(AnymalBRobotInterfaceArgs args) {
    logger_ = tbai::getLogger("tbai_deploy_anymal_b");
    TBAI_LOG_INFO(logger_, "AnymalBRobotInterface constructor (tbai_sdk/zenoh backend)");

    initMotorMapping();

    TBAI_LOG_INFO(logger_, "Initializing publisher: Topic: {}", ANYMAL_B_TOPIC_LOWCMD);
    lowcmd_publisher = std::make_unique<tbai::Publisher<robot_msgs::MotorCommands>>(ANYMAL_B_TOPIC_LOWCMD);
    ...
    TBAI_LOG_INFO(logger_, "Initializing subscriber - Topic: {}", ANYMAL_B_TOPIC_LOWSTATE);
    lowstate_subscriber = std::make_unique<tbai::QueuedSubscriber<robot_msgs::LowState>>(
        ANYMAL_B_TOPIC_LOWSTATE, [this](const robot_msgs::LowState &msg) { lowStateCallback(msg); }, 1);
}

AnymalBRobotInterface::~AnymalBRobotInterface() {
    TBAI_LOG_INFO(logger_, "Destroying AnymalBRobotInterface");
    if (lowstate_subscriber) lowstate_subscriber->stop();
}

void AnymalBRobotInterface::lowStateCallback(const robot_msgs::LowState &low_state) {
    ...
    vector_t jointAngles(ANYMAL_B_NUM_JOINTS);
    vector_t jointVelocities(ANYMAL_B_NUM_JOINTS);
    ...
    State state;
    state.x = vector_t::Zero(ANYMAL_B_STATE_DIM);
...
void AnymalBRobotInterface::publish(std::vector<MotorCommand> commands) {
    ...
    motor_commands.commands.resize(ANYMAL_B_NUM_JOINTS);
...
void AnymalBRobotInterface::waitTillInitialized() {
    while (!initialized_) {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        TBAI_LOG_INFO_THROTTLE(logger_, 1.0, "Waiting for the ANYmal B robot to initialize...");
    }
    TBAI_LOG_INFO(logger_, "ANYmal B robot initialized");
}

State AnymalBRobotInterface::getLatestState() {
    std::lock_guard<std::mutex> lock(latestStateMutex_);
    return state_;
}

void AnymalBRobotInterface::updateGroundPlaneEstimate() {
...
double AnymalBRobotInterface::estimateGroundHeight(const robot_msgs::PointCloud2 &pc) {
