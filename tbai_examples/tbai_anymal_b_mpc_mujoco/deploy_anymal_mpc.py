...
class UIController:
    ...
    def __init__(self, stand_callback, sit_callback, mpc_callback, set_gait_callback):
        self.root = tk.Tk()
        self.root.title("ANYmal MPC Controller")
...
CONFIG_FILE = os.path.join(SCRIPT_DIR, "anymal_mpc_config.yaml")

os.environ["TBAI_ROBOT_DESCRIPTION_PATH"] = str(tbai_descriptions.get_urdf_path("anymal_b"))
...
def main():
    ...
    if not tbai_python.HAS_DEPLOY_ANYMAL_D:
        print("Error: tbai was built without ANYmal B support (TBAI_BUILD_DEPLOY_ANYMAL_D=OFF)")
        sys.exit(1)

    robot_args = tbai_python.AnymalBRobotInterfaceArgs()
    print("Connecting to ANYmal B...")
    robot = tbai_python.AnymalBRobotInterface(robot_args)
    ...
    mpc_ctrl = tbai_python.QuadrupedMpcController(
        ...
        urdf_path=str(tbai_descriptions.get_urdf_path("anymal_b")),
