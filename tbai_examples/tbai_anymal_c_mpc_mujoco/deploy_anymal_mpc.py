...
os.environ["TBAI_ROBOT_DESCRIPTION_PATH"] = str(tbai_descriptions.get_urdf_path("anymal_c"))
...
def main():
    ...
    if not tbai_python.HAS_DEPLOY_ANYMAL_D:
        print("Error: tbai was built without ANYmal C support (TBAI_BUILD_DEPLOY_ANYMAL_D=OFF)")
        sys.exit(1)

    robot_args = tbai_python.AnymalCRobotInterfaceArgs()
    print("Connecting to ANYmal C...")
    robot = tbai_python.AnymalCRobotInterface(robot_args)
    ...
    mpc_ctrl = tbai_python.QuadrupedMpcController(
        ...
        urdf_path=str(tbai_descriptions.get_urdf_path("anymal_c")),
