...
os.environ["TBAI_ROBOT_DESCRIPTION_PATH"] = str(tbai_descriptions.get_urdf_path("anymal_d"))
...
def main():
    ...
    if not tbai_python.HAS_DEPLOY_ANYMAL_D:
        print("Error: tbai was built without ANYmal D support (TBAI_BUILD_DEPLOY_ANYMAL_D=OFF)")
        sys.exit(1)

    robot_args = tbai_python.AnymalDRobotInterfaceArgs()
    print("Connecting to ANYmal D...")
    robot = tbai_python.AnymalDRobotInterface(robot_args)
    ...
    mpc_ctrl = tbai_python.QuadrupedMpcController(
        ...
        urdf_path=str(tbai_descriptions.get_urdf_path("anymal_d")),
