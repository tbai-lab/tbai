...
CONFIG_FILE = os.path.join(SCRIPT_DIR, "spot_mpc_config.yaml")

os.environ["TBAI_ROBOT_DESCRIPTION_PATH"] = str(tbai_descriptions.get_urdf_path("spot"))
...
def main():
    ...
    if not tbai_python.HAS_DEPLOY_SPOT:
        print("Error: tbai was built without Spot support (TBAI_BUILD_DEPLOY_SPOT=OFF)")
        sys.exit(1)

    robot_args = tbai_python.SpotRobotInterfaceArgs()
    print("Connecting to Spot...")
    robot = tbai_python.SpotRobotInterface(robot_args)
    ...
    mpc_ctrl = tbai_python.QuadrupedMpcController(
        ...
        urdf_path=str(tbai_descriptions.get_urdf_path("spot")),
