...
std::unique_ptr<WbcBase> getWbcUnique(const std::string &controllerConfigFile, const std::string &urdfString,
    ...
    if (wbcType == "hqp") {
        return std::make_unique<HqpWbc>(controllerConfigFile, urdfString, comModel, kinematics, frameDeclaration);
    }

    if (wbcType == "sqp") {
        return std::make_unique<SqpWbc>(controllerConfigFile, urdfString, comModel, kinematics, frameDeclaration);
    }
...
std::shared_ptr<WbcBase> getWbcShared(const std::string &controllerConfigFile, const std::string &urdfString,
    ...
    return std::shared_ptr<WbcBase>(
        getWbcUnique(controllerConfigFile, urdfString, comModel, kinematics, frameDeclaration).release());
}
}  // namespace quadruped_arm
}  // namespace mpc
}  // namespace tbai
