...
ocs2::SystemObservation MpcController::generateSystemObservation() const {
    ...
    std::swap(observation.state(12 + 3), observation.state(12 + 6));
    std::swap(observation.state(12 + 4), observation.state(12 + 7));
    std::swap(observation.state(12 + 5), observation.state(12 + 8));

    // Set input: [contact_forces(12), leg_vels(12), arm_vels(6)]
    observation.input.setZero(inputSize);
    // Leg velocities from rbdState (after joint positions)
    observation.input.segment(12, numLegJoints) = rbdState.segment(12 + numLegJoints + numArmJoints, numLegJoints);
    // Arm velocities from rbdState
    observation.input.tail(numArmJoints) = rbdState.tail(numArmJoints);

    // Swap LH and RF for leg velocities
